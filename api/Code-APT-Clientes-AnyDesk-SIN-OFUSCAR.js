...
// @version      4.1.9
// @description  v4.1.9: Versión con BLOQUEO DE SEGURIDAD (Clave 11aa) + Fusión de módulos v-4.1.8.
// @author       Gemini
...
//  ---------------------------------------------------------------
//  Esta sección verifica la clave antes de cargar nada más.
//  Si la clave es incorrecta, el script MUERE aquí mismo.
// ===============================================================

const CLIENT_PASS = '11aa'; // CLAVE MAESTRA

(function() {
    'use strict';

    // --- 1. VALIDACIÓN DE SEGURIDAD ---
    // Recibimos las credenciales inyectadas por el lanzador
    var passIngresado = window.__APT_CLIENT_PASS__;
    var token         = window.__TYKO_SECURE_TOKEN__;

    // Borramos las huellas globales inmediatamente
    ...
    // A. Verificamos Token de origen (Lanzador)
    if (token !== 'ACCESO-TYKO-OK') {
        console.error('⛔ [APT-SEGURIDAD] Ejecución rechazada: No se detectó el lanzador oficial.');
        throw new Error('[APT-SEGURIDAD] ALTO: Intento de ejecución no autorizado.'); // DETIENE TODO
    }

    // B. Verificamos la CLAVE
    if (passIngresado !== CLIENT_PASS) {
        console.error('⛔ [APT-SEGURIDAD] Clave incorrecta: ' + passIngresado);
        alert('⛔ ACCESO DENEGADO ⛔\n\nLa clave de licencia es incorrecta.\nEl sistema se detendrá.');
        throw new Error('[APT-SEGURIDAD] ALTO: Clave incorrecta. Ejecución abortada.'); // DETIENE TODO
    }

    // Si pasa, mostramos luz verde en consola y continuamos
    console.log('%c✅ [APT-SEGURIDAD] Clave correcta (11aa). Cargando Sistema v-4.1.9...', 'color: #00FF00; font-weight:bold;');


    // ===============================================================
    //  🔻 INICIO DEL CÓDIGO FUNCIONAL (v-4.1.8 Original) 🔻
    // ===============================================================
    ...
    (function bootstrap() {
        ...
        (function() {
            ...
            document.addEventListener('click', async function(ev) {
                ...
                try {
                    ...
                    const a = document.createElement('a'); a.href = blobUrl; a.download = nombreFinal;
                    document.body.appendChild(a);
                    a.click(); a.remove(); URL.revokeObjectURL(blobUrl);
                } catch (e) {
                    pdfLog('Fallo descarga renombrada:', e); window.location.href = btn.href;
                }
            }, true);
        })();
        ...
        function iniciarModuloAptPaneles() {
            ...
            const urlsPermitidas = [ 'https://apt.cfia.or.cr/APT2/Home', 'https://apt.cfia.or.cr/APT2/Plano/Consulta' ];
            ...
            function findLogoCFIA() { let img = q('img[src*="Logo-CFIA-esencial"], img[alt*="LogoCFIA"]'); if (img) return img; return Array.from(document.images).find(function(im) { return /logo[-_]?cfia/i.test(im.src) || /LogoCFIA/i.test(im.alt); }) || null; }
            function findBarraColegios() { let img = q('img[src*="Logos_Coleg"], img[src*="Logos-Coleg"], img[alt*="LogoColeg"]'); if (img) return img; return Array.from(document.images).find(function(im) { return /logos[_-]?coleg/i.test(im.src) || /LogoColeg/i.test(im.alt); }) || null; }
            ...
            function createControlPanel(targets) {
                ...
                css.textContent = `
                    ...
                    font-family: system-ui, Arial, sans-serif; font-size: 13px; backdrop-filter: blur(3px); width: auto; min-width: 680px; margin-left: 20px; padding: 6px; transform: scale(calc(var(--panel-ajustes-scale) / var(--panel-scale)));
            ...
            try { new ResizeObserver(function() { aplicarFluido(); applyPanelScale(); applyPanelAjustesScale(); }).observe(document.documentElement); } catch (e) {}
        ...
        (function() {
            ...
            (function() {
                ...
                if (!document.getElementById(estiloID)) {
                    ...
                    estilos.id = estiloID;
                    estilos.textContent = `
                        .linea-unificada { display: flex; gap: 10px; align-items: center; justify-content: flex-start; font-size: 13px; color: #495057; padding: 3px 0; margin-bottom: 8px; }
                        ...
                        #ULArchivos li.list-group-item { padding-top: ${ESPACIO_LI_VERTICAL} !important; padding-bottom: ${ESPACIO_LI_VERTICAL} !important; border-bottom: ${GROSOR_BORDE} solid ${COLOR_BORDE} !important; border-top: none !important; }
            ...
            (function() {
                ...
                function gestionarTipoArchivoPorDefecto() {
                    ...
                    if (!select || !visible(select) || document.activeElement === select) { return; }
                ...
                function runAnchoCompletoArchivos() {
                    ...
                    if (mainContainer) { mainContainer.classList.remove('col-md-6'); mainContainer.classList.add('col-md-12'); }
                ...
                function iniciarAutoCargarArchivo() {
                    ...
                    function clickCargarArchivo(titulo) {
                        ...
                        if (!btn) { log('[Gestor Archivos][Auto Cargar] Botón #btnCargarArchivo no encontrado.'); return; }
                        ...
                        var observer = new MutationObserver(function(mutations) {
                            mutations.forEach(function(m) {
                                if (m.type === 'attributes' && m.attributeName === 'title') {
                                    ...
                                    setTimeout(function() { clickCargarArchivo(titulo); }, 400);
                                }
                            });
                        });
                        observer.observe(campo, { attributes: true, attributeFilter: ['title'] });
                    ...
                    var intentos = 0; var maxIntentos = 15;
                    var timer = setInterval(function() {
                        intentos++;
                        if (buscarCamposIniciales() || intentos >= maxIntentos) { clearInterval(timer); }
                    }, 700);
                        ...
                        mutations.forEach(function(m) {
                            if (m.type === 'childList' && m.addedNodes.length > 0) {
                                Array.prototype.forEach.call(m.addedNodes, function(node) {
                                    ...
                                    if (node.matches && node.matches('input.file-caption-name')) { procesarCampo(node); }
                                    if (node.querySelectorAll) {
                                        var internos = node.querySelectorAll('input.file-caption-name');
                                        if (internos.length > 0) { internos.forEach(procesarCampo); }
                                    }
                                });
                            }
                        });
                ...
                function escanearYActualizar() {
                    if (typeof window.runArchivosEnUnaLinea !== 'function') { log('Error: Módulo "runArchivosEnUnaLinea" no encontrado.'); return; }
                ...
                setInterval(function() {
                    ...
                    if (panelArchivos && panelArchivos.classList.contains('show')) {
                        if (!panelArchivosVisible) {
                            ...
                            if (listaArchivos) { observerArchivos.observe(listaArchivos, { childList: true }); }
                        } else {
        ...
        (function () {
            ...
            function log(msg) { console.log(`[APT Trámite ${VERSION}] ${msg}`); }

            function normalizarTexto(str) { return (str || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase().trim(); }
            ...
            function obtenerTextoTramite() {
                ...
                for (const el of candidatos) {
                    ...
                    if (texto.startsWith('Trámite:')) { return texto; }
                }
                return null;
            }
            ...
            function copiarAlPortapapeles(texto) {
                ...
                } catch (e) { console.error('[APT Trámite] Excepción al copiar:', e); }
            }

            function intentarCapturaTramite() {
                if (yaCapturado) return;
                const menuPlanos = document.querySelector('#plano-tab');
                if (!menuPlanos) { return; }

                const valorArea = obtenerValorAreaTotal();
                if (valorArea === null) { log('Área Total aún no detectada, reintentando...'); return; }
            ...
            temporizador = setInterval(() => {
                ...
                if (yaCapturado || intentos > MAX_INTENTOS) {
                    if (temporizador) { clearInterval(temporizador); temporizador = null; }
                    if (!yaCapturado) { log('No se logró encontrar condición (Área=0 + Trámite) dentro del número máximo de intentos.'); }
                    return;
                }
                intentarCapturaTramite();
            }, INTERVALO_MS);
            window.addEventListener('load', () => { if (!yaCapturado) { intentarCapturaTramite(); } });
        ...
        (function() {
            var logLocal_ENT = (typeof log === 'function') ? function(msg) { log('[Entero Popup] ' + msg); } : function(msg) { console.log('[Entero Popup] ' + msg); };
            ...
            function autoClickConfirm_ENT() {
                ...
                observer_ENT.observe(document.body, { childList: true, subtree: true });
            }
        ...
        function setTotalCFIA() { const input = document.getElementById('txtTotalCFIA'); const valor = '1600';
            if (input && visible(input)) { if (input.value.trim() === '') { log(`Planos/Enteros: Estableciendo valor por defecto de Total CFIA: "${valor}"...`); input.value = valor; fireChangeLike(input); } return true; } return false;
        }
        function setMontoCIT() { const input = document.getElementById('txtMontoCIT_NTRIP'); const valor = '300';
            if (input && visible(input)) { if (input.value.trim() === '') { log(`Planos/Enteros: Estableciendo valor por defecto de Monto CIT-NTRIP: "${valor}"...`); input.value = valor; fireChangeLike(input); } return true; } return false;
        }
